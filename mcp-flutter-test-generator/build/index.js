...
class FlutterTestGeneratorServer {
    ...
    async handleGenerateTestSuite(args) {
        ...
        return {
            content: [
                {
                    type: "text",
                    text: `# Generated Test Suite\n\n**Coverage Level:** ${coverage}\n**Total Tests:** ${testSuite.tests.length}\n\n${testSuite.tests.map((test, idx) => `## Test ${idx + 1}: ${test.file}\n\n\`\`\`dart\n${test.content}\n\`\`\``).join('\n\n')}`,
                },
            ],
        };
    }
    async handleAnalyzeWidget(args) {
